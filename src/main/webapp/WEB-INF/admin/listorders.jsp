					...
					<table class="table table-bordered table-striped table-hover">
						...
						<c:forEach items="${ordersList}" var="orders">
							<tr>
								...
								<td style="text-align: center;"><c:if test="${orders.status eq'预约'}">
										<a href="orders/lend.action?id=${orders.ordersid}">借出</a>&nbsp;&nbsp;
							</c:if> <c:if test="${orders.status eq'借阅'}">
										<a href="orders/back.action?id=${orders.ordersid}">归还</a>&nbsp;&nbsp;
							</c:if><a href="orders/deleteOrders.action?id=${orders.ordersid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}"><i
